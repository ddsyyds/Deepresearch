...
# 创建文件处理器
file_handler = logging.FileHandler('altitude_mcp_server.log',encoding='utf-8')
...
def web_search(query: str, top_k: int = 2, categories: str = 'general') -> str:
    ...
    response = requests.get(
        f'http://127.0.0.1:8080/search?format=json&q={query}&language=zh-CN&time_range=&safesearch=0&categories={categories}',
        timeout=30)
    if response.status_code != 200:
        logger.info("response error:400,url address maybe have a question")
        return ""
    else:
